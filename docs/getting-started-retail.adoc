...
Sources are JSON under the FILE catalog root (`work/edw-catalog/` for retail); the namespace in each file must match its folder path (`hop/retail-example/sources`). Initial setup runs `scripts/bootstrap-retail-work.py` to create the `work/` tree, generate E2E DATABASE sources, and seed COMPOSITE / JSON / PIPELINE feeds (`all-customer-info`, `feed_order_shipment_tracking`, `asn-package-lines`) that `retail-360.hdv` requires.
