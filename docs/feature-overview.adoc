...
**Data Hopper EDW** is a set of Apache Hop plugins for an **Enterprise Data Warehouse**: catalog, source models, **model-driven Data Vault 2.0**, **Business Vault**, and **dimensional** loading. Version **0.11.0-SNAPSHOT** (latest release **0.10.0**) requires **Apache Hop 2.20.0-SNAPSHOT** and **Java 21**.
...
**Build an EDW:** link:getting-started-edw.adoc[Getting started — building an EDW]. **Architecture** (catalog, resource definition group, four controls): link:architecture.adoc[Architecture]. **Tour the sample:** link:getting-started-retail.adoc[Getting started — retail]. **AdventureWorks demo** (SQL Server source → Postgres DV/BV/DM): link:getting-started-adventureworks.adoc[Getting started — AdventureWorks] (issue https://github.com/ProjectDataHopper/hopper-edw/issues/180[#180]).
...
|Diagram export (Hop 2.20)
|Available (SVG, PDF, PlantUML, Draw.io, Mermaid for `.hsm` / `.hdv` / `.hbv` / `.hdm` / `.hem`)
|link:README.md[docs README] (`hop export`), issue https://github.com/ProjectDataHopper/hopper-edw/issues/175[#175]

|Project documentation (static HTML)
|Available (Hop Web: explorer + RAP handler, issue https://github.com/ProjectDataHopper/hopper-edw/issues/169[#169])
|link:project-documentation.adoc[Project documentation], issue https://github.com/ProjectDataHopper/hopper-edw/issues/25[#25]

|Kimball bus matrix
|Available
|link:bus-matrix.adoc[Bus matrix], issue https://github.com/ProjectDataHopper/hopper-edw/issues/177[#177]
...
|BV bridge tables (hub hash keys, optional weight / SQL)
|Preview
|link:business-vault-bridge.adoc[Business Vault bridge]
...
|Logical dimensional contracts (source type None)
|Available
|Sketch dims/facts without a load source; Check model warns; Dimensional Update skips. link:dimensional-modeler-overview.adoc[Dimensional modeler]

|Dimensional field notes and requirements (Markdown)
|Available
|Field editor on attributes, keys, measures, degenerates; project-doc renders Markdown. link:help/dm-field-dialog.adoc[Field editor]
...
|Hopper presentations in Hop GUI
|Preview (issues https://github.com/ProjectDataHopper/hopper-edw/issues/170[#170], https://github.com/ProjectDataHopper/hopper-edw/issues/185[#185])
|Generated dashboards (no web server, no Hop metadata editors) via **Tools → Project presentation…** and explorer **View execution presentation** on `.hpl`/`.hwf`. Pipeline results **Performance** tab (throughput over time) and workflow results **Gantt** tab (action durations). Built on the fly with `HSimplePresentation`; drawn by `HPresentationViewer` / `HPresentationSession` (Hop Web: inline SVG + JS). Semantic layer **Create presentation...** (or a `.hdm` fact shortcut) opens the link:help/fact-crosstab-editor.adoc[crosstab editor] and generates a SQL star-join crosstab (groups / composite when requested).

|Semantic layer (`.hsl`)
|Preview
|Consumption metadata over `.hdm`: entities, measures (default format masks), relationships, named selections, Calcite SQL compile. Tree editor binds a dimensional model (several `.hsl` per `.hdm`) and **Create presentation...**. Crosstab save/open selection. link:semantic-layer.adoc[Semantic layer].
...
|Available (DV/BV/DM proposals; `.hsm` / `.hlv` / `.hem` / Journey chat-only)
...
|Available
|link:synthetic-data.adoc[Synthetic data]
...
|`hop project-doc` export
|Available
|link:project-documentation.adoc[Project documentation]

|EDW naming schemes
|Available
|Hop 2.20 Naming Schemes for models, tables, physical names, and record definitions; link:naming-schemes.adoc[EDW naming schemes]

|BV semantic naming rules
|Planned
|Vague tokens, language, acknowledgements; link:plans/bv-naming-rules-engine-plan.md[BV naming rules plan] (mechanical schemes first)
...
* **Synthetic data** — seeded rows (sequences, choices, parent/child, unique pairs, optional nested XML). Retail source waves use `retail-example/pipelines/generate-retail-data.hpl`.
...
image::images/action-update-resource-group-load-metrics-dialog.png[Resource group load metrics — live wave progress and transform metrics,align="center"]
