...
Raw Data Vault and generated SCD2/PIT tables cover the mechanical patterns. Real projects still need arbitrary SQL for business rules, enrichments, and lookups. dbt handles this with `ref()` / `source()` and dependency ordering. Data Hopper EDW brings the same idea onto the `.hbv` model:
