...
| Ops metrics database / schema | Hop connection (and optional schema) for `load_pipeline_metric`. Blank schema inherits the Execution metrics profile (retail: `dv_ops`)
...
* **Ops enrichment skipped** — configure an ops database. Leave **Ops schema** blank to inherit the Execution metrics profile (retail: `dv_ops` on **OPS**). If the log says `relation "load_pipeline_metric" does not exist`, the query ran against the connection default schema instead of `dv_ops`, or the tables were never created. Open the **Execution metrics profile** and use **Generate SQL** to review or execute the dialect-specific `CREATE` statements. Enrichment never fails the export by itself.
