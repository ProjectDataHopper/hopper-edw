...
Data Hopper EDW captures **source-to-target lineage** for every Data Vault (DV), Business Vault (BV), and dimensional (DM) target table and field. Lineage answers three questions that production gates and opaque model generators often leave open:
...
Data Hopper EDW avoids that opacity by:
...
| `BV_SCD2_HUB_BK` | Parent hub business key copied onto a BV SCD2 table
...
| link:hop-lineage-view.adoc[Hop Lineage View] (`.hlv`) | In-GUI **upstream** graph over Marquez, an export folder, or local models. Same collectors; not a second place to edit mappings
| link:execution-maps.adoc[Execution maps] (`.hem`) | Workflow / pipeline / dataset *execution* graph; not field-level mapping reasons
| Marquez / OpenLineage | External table- and column-level job lineage via **Export data lineage** action — link:openlineage-export.adoc[OpenLineage export] (screenshots of the action and Marquez graphs)
