...
* Name-click updates the right-hand details pane. On Hop Web that pane is plain text (RAP has no `StyledText`); use **View as HTML** for formatted markdown. Card-click opens the context dialog after an unmoved mouse-up (same RAP click-vs-drag split as the other modelers).
