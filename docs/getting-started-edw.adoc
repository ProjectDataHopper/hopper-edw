= Getting started — building an EDW with Data Hopper EDW
:toc: macro
:toclevels: 3
...
To **run a finished warehouse** instead of building one, use link:getting-started-retail.adoc[Getting started — retail example] or link:getting-started-adventureworks.adoc[Getting started — AdventureWorks]. Advanced fixtures: link:getting-started-integration-tests.adoc[integration test fixtures].
...
. Add SCD2 / PIT / bridge / SQL business tables. **Check model**.
...
. **Edit model** — configuration `dimensional`. You can sketch dimensions and facts with source type **None (logical)** and field Markdown notes before BV/DV sources exist. Configure a load source (SQL, pipeline, record definition, or date generator) before Dimensional Update.
. **Check model**.
