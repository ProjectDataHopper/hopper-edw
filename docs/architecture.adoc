...
This page is the **Data Hopper EDW architecture** picture: what the Data Catalog is for, how the model files relate, and which workflow actions sit on that spine.
...
|Business Vault
|`.hbv`
|SCD2, PIT, bridges, SQL business tables on top of raw vault history.
...
|Semantic layer
|`.hsl`
|Consumption model over the warehouse: entities, attributes, measures, relationships, format masks, named selections. Bound to a `.hdm` (several `.hsl` per model). Used by fact crosstabs and later presentations.
