= Get Record Definition Names
:toc: macro
:toclevels: 3

toc::[]

The **Get Record Definition Names** transform (plugin id `RecordDefinitionInput`) reads record definitions from a Hop **data catalog** connection and outputs their **names and metadata** as pipeline rows (optionally one row per field). It does **not** read source data rows — for that use link:record-definition-data-input.adoc[Record Definition Input].
...
image::images/record-definition-input-dialog-general-settings.png[Get Record Definition Names dialog — General Settings tab,align="center"]
...
|Namespace value
|Enabled when **Select from previous transform input?** is off. Optional namespace. Leave empty together with **Name value** to list every working-tree record. Set this without **Name value** to list a namespace prefix. Version snapshots are not listed.
...
|Fail if no record definitions found?
|When disabled (default), an empty working-tree list succeeds with 0 rows (and a log warning). When enabled, the transform fails instead. Use this on pipelines such as retail `create-source-tables-from-data-catalog.hpl` so a missing bootstrap or a catalog pointed at snapshots is not silent. **Catalog sources versions** snapshots are never listed.

|Preview catalog contents
|Opens a dialog with the resolved FILE storage directory, working-tree count, skipped unreadable JSON files, and a table of namespace / name / type. Version snapshots are not shown.
|===

=== Selection modes

The transform supports these ways to choose which definitions to read:

[cols="1,1,3", options="header"]
|===
|Select from input |Namespace / name values |Behavior

|No |Both empty |List every *working-tree* record definition in the catalog connection (not version snapshots) and output one row per definition (or one row per field when denormalized).

|No |Namespace set, name empty |List working-tree records whose namespace starts with the given value (prefix).

|No |Name set (with or without namespace) |Read the single definition identified by the given namespace and name.
...
When **Select from previous transform input?** is enabled, upstream transforms must supply the namespace and name fields before **Get Record Definition Names** runs. A **Get variables** or **Get records from stream** step works well when values come from pipeline parameters.
...
image::images/record-definition-input-dialog-output-fields.png[Get Record Definition Names dialog — Output Fields tab,align="center"]
...
The sample project uses **Get Record Definition Names** together with the **DDL** transform to prepare CRM source tables automatically before the Data Vault test suites run.
...
1. **Get Record Definition Names** — catalog connection `local-catalog`, no input selection, no namespace/name filter (all working-tree definitions), denormalized output off. Enable **Fail if no record definitions found?** when an empty catalog should stop the pipeline.
...
2. **Get Record Definition Names** — **Select from previous transform input?** enabled; reads namespace and name from input; **Output fields metadata** enabled so each column becomes its own row.
...
* After importing tables into the catalog from the Data Catalog perspective, re-run pipelines that depend on **Get Record Definition Names** so DDL reflects the latest field layout.
