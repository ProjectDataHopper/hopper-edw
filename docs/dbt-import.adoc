...
Imported `{% macro %}` blocks are stored as a **Jinja macro library** in project metadata (Metadata perspective → **EDW**). Enable the library, set optional default `var()` values, and use **Test render** to expand a snippet with dummy `ref` / `source` names.
...
SCD2-like dbt models (historization plus `CASE` / `COALESCE` / `CAST`) should not stay as imported SQL. Rebuild them as a BV SCD2 table with **Calculations** — see link:business-vault-scd2.adoc[Business Vault SCD2].
...
See link:business-vault-sql-view.adoc[Business Vault SQL] and issue https://github.com/ProjectDataHopper/hopper-edw/issues/72[#72].
