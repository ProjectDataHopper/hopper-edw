...
|**Parent hub**
|The hub this SCD2 timeline is for. Linked satellites and source queries are all sorted and merged on that hub's hash key plus the functional timestamp. Origin does not matter. Leave empty to infer from linked satellites. Set this when the SCD2 is fed only by source queries. **Validate** / Check model reports an error if the name is not a Linked Hub or a hub in the loaded Data Vault model.
...
**Suggest mappings** reloads the linked `.hdv` from disk, then adds one row per unmapped satellite attribute **and** per unmapped source-query column (hash key, functional timestamp, and load date on the source query are skipped). Target names stay unique on collisions. If nothing is added, a message explains why: Data Vault model not loaded (satellites only), satellite not in that model, source query not on the canvas, the satellite has no attributes (use **Get attributes**), or the source query has no columns (use **Get fields**). You do not need to close the BV tab after editing the `.hdv` or a source query.
