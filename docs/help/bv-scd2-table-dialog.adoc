...
**Load** (row checkbox, default checked): Unchecked keeps the column on the stream for SQL calculations and versioning but does not write it to the BV table. Unchecked Load is full rebuild only. The Lineage tab lists every loaded column, including hub business keys when those are loaded.

Single-satellite tables can pass attributes through under DV names unless you add explicit mappings for renaming, subsetting, or calculation-only columns.
