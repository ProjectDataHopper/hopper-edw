...
|**Parent hub**
|The hub this SCD2 timeline is for. Linked satellites and source queries are all sorted and merged on that hub's hash key plus the functional timestamp. Origin does not matter. Leave empty to infer from linked satellites. Set this when the SCD2 is fed only by source queries.
