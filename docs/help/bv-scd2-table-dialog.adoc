...
|When enabled, parent-hub business key columns are merged via Sorted schema merge. They are stored on the BV table and available for calculations, but they do not drive new versions. Requires Include hash key and a shared hub parent.
...
**Load** (Yes/No, default Yes): No keeps the column on the stream for SQL calculations and versioning but does not write it to the BV table. Load=No is full rebuild only.
