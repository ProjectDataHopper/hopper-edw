...
|When enabled, parent-hub business key columns are left-outer Merge Joined after collapse, immediately before SQL calculations. They are available for calculations but they do not drive new versions. Requires Include hash key and a shared hub parent. The SCD2 merge/repeat/incremental path is unchanged.
...
**Suggest mappings** reloads the linked `.hdv` from disk, then adds one row per unmapped satellite attribute (unique target names on collisions). If nothing is added, a message explains why: Data Vault model not loaded, satellite not in that model, or the satellite has no attributes (use **Get attributes** on the satellite, then suggest again). You do not need to close the BV tab after editing the `.hdv`.

The Field mappings tab shows how many tables were loaded and how many attributes each linked satellite has.
