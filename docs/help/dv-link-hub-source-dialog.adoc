...
|Column on the hub definition (e.g. `order_id`)
|Column in the link source (e.g. `order_id` or `cust_order_fk`)
...
|Target driving key name (e.g. `FROM_LOCATION_HK`)
|Source column supplying that value
