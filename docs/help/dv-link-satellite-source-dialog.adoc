...
|Attribute on the link satellite (e.g. `quantity`)
|Column in this source (e.g. `qty_ordered`)
...
|Driving key on the satellite (e.g. `line_number`)
|Source column (e.g. `line_number`)
...
|Hub business keys from relationship row
|**Hub sources**
|Link table

|Descriptive attributes on the relationship
|**Satellite sources**
...
|Hub source
|`order_id` → hub_order, `product_id` → hub_product

|Satellite source
|`quantity`, `unit_price`, `discount_pct` → satellite attributes; `line_number` → driving key
