...
* **Error** — type mismatch; source string/binary length or numeric precision exceeds the target definition; target database is not Unicode-capable for EDW string storage (SQL Server without a `_UTF8` collation, PostgreSQL non-UTF8 encoding, MySQL/SingleStore without `utf8mb4`). On **SQL Server**, length overflow is judged against the vault **UTF-8 byte capacity** (model character length ×3, matching generated `VARCHAR`), not the raw model number alone — so model length 50 (characters) is capacity 150, same as vault DDL.
