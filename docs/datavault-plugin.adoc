...
* **Error** — type mismatch; source string/binary length or numeric precision exceeds the target definition; target database is not Unicode-capable for EDW string storage when enforcement is enabled (SQL Server without a `_UTF8` collation, PostgreSQL non-UTF8 encoding, MySQL/SingleStore without `utf8mb4`; plain MySQL `utf8` is incomplete UTF-8). Enforcement is a global option under **Configuration → Data Vault 2.0** (default on; off softens to a warning for migrations). On **SQL Server**, length overflow is judged against the vault **UTF-8 byte capacity** (model character length ×3, matching generated `VARCHAR`), not the raw model number alone — so model length 50 (characters) is capacity 150, same as vault DDL.
