...
1. Define **Data Vault Sources** that describe where your raw data lives (database connection, table, column layout, and record-source semantics). Optionally model multi-table joins in a **source model** (`.hsm`) and publish composite feeds.
2. Create **Hub**, **Link**, and **Satellite** definitions and group them into a **Data Vault Model** (saved as a `.hdv` file). From a source model you can **Generate Data Vault…** to propose those objects from primary keys and relationships (review-and-apply). Prefer a shared **Data Vault configuration** (Tools → **Configure EDW setup...**, or the first-time project prompt) and point each model at it.
...
6. In a workflow, add the **Data Vault Update** action (or **Update resource definition group** for multi-model waves), point it at your model(s), choose a pipeline run configuration, and run the full load (with optional model checks and DDL handling).
