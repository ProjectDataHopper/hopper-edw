...
2. Create **Hub**, **Link**, and **Satellite** definitions and group them into a **Data Vault Model** (saved as a `.hdv` file). From a source model you can **Generate Data Vault…** to propose those objects from primary keys and relationships (review-and-apply). Each model carries its own embedded **configuration** (target database, hashing rules, column names, sentinel records, and pipeline options).
