...
Without a source modeler, Data Hopper EDW typically maps each physical table to its own catalog `DV_SOURCE` and often its own satellite. That is correct for true independent history, but expensive when one satellite (or hub load) should read a join.
...
Sample retail pipeline: `retail-example/pipelines/parse-asn-xml.hpl` + `files/asn_demo.xml` (and wave files from `pipelines/generate-retail-data.hpl`).
...
* Driver: `org.hopper.edw.datavault.virtualization.jdbc.HopSourceModelJdbcDriver`
...
With Data Hopper EDW installed, create a connection of type **Apache Hop Source Model** (`HOPSOURCEMODEL`):

|===
| Hop field | Maps to |

| Host name | Hop Server / Hop Web host (`localhost` when the Database perspective runs inside Hop Web) |
| Port number | HTTP port (Hop Server often *8888*; Hop Web Tomcat *8080*) |
| Database name | Optional default Source model service (JDBC schema) |
| Username / password | Basic: Hop Server user/password. Bearer: leave user empty and paste a JDBC token as the password |
| Authentication | *Basic*, *Bearer*, or *OAuth 2* (driver fetches a token; not used with Google OIDC) |
|===
...
Hop Server must load Data Hopper EDW (servlet path `/hop/sourceModelData`) and have RDBMS connection metadata for the model’s sources.
...
| Username / Password | Hop Server credentials | Basic auth. For Bearer, put the token in *Password* (do not put it in the URL) |
| Driver properties | `authType=basic\|bearer\|oauth2` | Optional. OAuth 2 also needs `oauthTokenUrl`, `oauthClientId`, `oauthClientSecret` / `oauthScope` |
|===
...
Do **not** put a Bearer token in the URL userinfo. Use the password field or the `accessToken` property.

On Hop Web with OAuth2 (for example https://hop-web.data-hopper.com/ui): **File → Copy JDBC token**, then use connection `hsm-crm-web` (host `localhost`, port `8080`, database `crm`, Authentication *Bearer*, paste the token as the password). Open the Database perspective and connect. The token lasts about an hour; do not save it in project metadata.
...
* **Catalog publish is current** — when a table / query / JSON / pipeline card already has a catalog feed name and a catalog connection, Check compares the card's **effective** layout (including data type mappings) to the published `DV_SOURCE`. A stale catalog is an **error**: the canvas is not the contract vault loads use. Publish the card (or **Push to catalog**), then re-harvest / re-validate.

The same catalog-stale check runs from **Validate** on the table, query, JSON, and pipeline dialogs.

Saving the `.hsm` also compares published feeds. When any published catalog feed lags the canvas, a dialog asks **Do you want to publish these?** (with **Don't remind me after save**). The reminder is on by default under **Configuration → Data Vault 2.0**. Never-published draft cards are not included.
...
== Generate a Data Vault model

Toolbar **Generate Data Vault…** (or the canvas context action) classifies selected tables, queries, JSON extractions, and pipeline cards from their primary keys and relationships and adds hubs, satellites, links, and reference tables to a new or existing `.hdv` after a review screen. Isolated tables with no relationships are skipped; JSON and pipeline children of those tables can still generate. Full walkthrough: link:generating-data-vault-from-source-model.adoc[Generate a Data Vault from a source model].
...
image::images/source-modeler-generate-data-vault-model-dialog.png[Review dialog proposing hubs, satellites, and links from the source tables,align="center"]
...
* link:generating-data-vault-from-source-model.adoc[Generate a Data Vault from a source model]
