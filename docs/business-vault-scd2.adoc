...
Each generated `TableInput` uses that source's Hop connection. Table/view names are read directly. Authored SQL is wrapped as a subquery so the plugin can add partition `WHERE` and hash-key/date `ORDER BY` outside the inner statement. The source query must name its **hash key field**.

Mixed SCD2 (DV satellites plus source queries) is supported; two or more inputs still require field mappings. The mapping **Source** column is the satellite name or the source-query name.
