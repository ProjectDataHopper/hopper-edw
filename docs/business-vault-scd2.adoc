...
|Field mappings Load checkbox
|Checked (default) writes the mapped column to the BV table. Unchecked keeps it on the stream for calculations and versioning only. Full rebuild only — Incremental plus Load unchecked is a Check-model error.
