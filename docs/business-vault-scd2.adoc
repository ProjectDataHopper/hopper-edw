...
Each generated `TableInput` uses that source's Hop connection. Table/view names are read directly. Authored SQL is wrapped as a subquery so the plugin can add partition `WHERE` and hash-key/date `ORDER BY` outside the inner statement.
