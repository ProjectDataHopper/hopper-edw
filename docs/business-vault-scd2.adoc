...
* Optional **Include hub business keys** (General tab) merges parent-hub BK columns through Sorted schema merge. They are identity fields: available for calculations and stored on the table, but they do not open new versions.
...
|When enabled, parent-hub business key columns are merged in as an extra Sorted schema merge input. Identity only (not version drivers). Requires Include hash key and hub-parent satellites. Not supported for link satellites or driving keys.

|Field mappings Load column
|**Yes** (default) writes the mapped column to the BV table. **No** keeps it on the stream for calculations and versioning only. Full rebuild only — Incremental plus Load=No is a Check-model error.
