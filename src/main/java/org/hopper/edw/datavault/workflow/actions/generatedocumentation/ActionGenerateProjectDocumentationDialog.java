...
public class ActionGenerateProjectDocumentationDialog extends ActionDialog {

  private static final Class<?> PKG = ActionGenerateProjectDocumentation.class;

  private final ActionGenerateProjectDocumentation action;
  private boolean cancelled = true;
  private GuiCompositeWidgets widgets;
  private Composite wSettingsComp;

  public ActionGenerateProjectDocumentationDialog(
      Shell parent,
      ActionGenerateProjectDocumentation action,
      WorkflowMeta workflowMeta,
      IVariables variables) {
    super(parent, workflowMeta, variables);
    this.action = action;
    if (Utils.isEmpty(action.getName())) {
      action.setName(BaseMessages.getString(PKG, "ActionGenerateProjectDocumentation.Name"));
    }
  }
  ...
  public IAction open() {
    createShell(
        BaseMessages.getString(PKG, "ActionGenerateProjectDocumentation.Title", action.getName()),
        action);

    buildButtonBar().ok(e -> ok()).cancel(e -> cancel()).build();

    DialogHelpSupport.installLocalHelpButton(
        shell, HelpTopics.ACTION_GENERATE_PROJECT_DOCUMENTATION);
    ...
    try {
      ...
      widgets.createCompositeWidgets(
          ...
          null,
          wSettingsComp,
          ActionGenerateProjectDocumentation.GUI_PLUGIN_ELEMENT_PARENT_ID,
          null);
      setWidgetsContent();
      ...
      new ErrorDialog(
          shell,
          BaseMessages.getString(PKG, "ActionGenerateProjectDocumentation.Name"),
          "Unable to create Generate project documentation settings widgets",
  ...
  private void ensureGuiElementsRegistered() {
    ...
    String parentId = ActionGenerateProjectDocumentation.GUI_PLUGIN_ELEMENT_PARENT_ID;
    ...
    LogChannel.UI.logBasic(
        "Generate project documentation: registering GUI widgets for " + className);
  ...
  private void setWidgetsContent() {
    ...
    if (widgets != null) {
      widgets.setWidgetsContents(
          action, wSettingsComp, ActionGenerateProjectDocumentation.GUI_PLUGIN_ELEMENT_PARENT_ID);
    }
  }
  ...
  private void getWidgetsContent() {
    ...
    if (widgets != null) {
      widgets.getWidgetsContents(
          action, ActionGenerateProjectDocumentation.GUI_PLUGIN_ELEMENT_PARENT_ID);
    }
  }
