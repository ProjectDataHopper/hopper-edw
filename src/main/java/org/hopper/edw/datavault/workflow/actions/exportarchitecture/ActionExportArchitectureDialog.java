 ...
 */
package org.hopper.edw.datavault.workflow.actions.exportarchitecture;
...
public class ActionExportArchitectureDialog extends ActionDialog {

  private static final Class<?> PKG = ActionExportArchitecture.class;

  private final ActionExportArchitecture action;
  private boolean cancelled = true;
  private GuiCompositeWidgets widgets;
  private Composite wSettingsComp;

  public ActionExportArchitectureDialog(
      Shell parent,
      ActionExportArchitecture action,
      WorkflowMeta workflowMeta,
      IVariables variables) {
    super(parent, workflowMeta, variables);
    this.action = action;
    if (Utils.isEmpty(action.getName())) {
      action.setName(BaseMessages.getString(PKG, "ActionExportArchitecture.Name"));
    }
  }
  ...
  public IAction open() {
    createShell(
        BaseMessages.getString(PKG, "ActionExportArchitecture.Title", action.getName()), action);

    buildButtonBar().ok(e -> ok()).cancel(e -> cancel()).build();

    DialogHelpSupport.installLocalHelpButton(shell, HelpTopics.ACTION_EXPORT_ARCHITECTURE);
    ...
    try {
      ...
      widgets.createCompositeWidgets(
          action, null, wSettingsComp, ActionExportArchitecture.GUI_PLUGIN_ELEMENT_PARENT_ID, null);
      ...
      new ErrorDialog(
          shell,
          BaseMessages.getString(PKG, "ActionExportArchitecture.Name"),
          "Unable to create Export Architecture settings widgets",
  ...
  private void ensureGuiElementsRegistered() {
    ...
    String parentId = ActionExportArchitecture.GUI_PLUGIN_ELEMENT_PARENT_ID;
  ...
  private void setWidgetsContent() {
    ...
    if (widgets != null) {
      widgets.setWidgetsContents(
          action, wSettingsComp, ActionExportArchitecture.GUI_PLUGIN_ELEMENT_PARENT_ID);
    }
  }
  ...
  private void getWidgetsContent() {
    ...
    if (widgets != null) {
      widgets.getWidgetsContents(action, ActionExportArchitecture.GUI_PLUGIN_ELEMENT_PARENT_ID);
    }
  }
