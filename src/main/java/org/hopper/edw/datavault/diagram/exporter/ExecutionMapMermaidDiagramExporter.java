...
@DiagramExporter(
    id = "execution-map-mermaid",
    name = "Execution map Mermaid Exporter",
    description = "Exports an execution map as a Mermaid flowchart",
    format = "MERMAID",
    fileExtension = "mmd",
    fileFilterNames = {"Mermaid Diagrams (*.mmd)"},
    supportedSubjectTypes = {ExecutionMapDocument.class})
public class ExecutionMapMermaidDiagramExporter
  ...
  public DiagramExportResult export(
    ...
    String content =
        ExecutionMapFlowRenderer.renderMermaid(
            ArchitectureGraphFromExecutionMap.build(document), SchemaRenderOptions.from(options));
