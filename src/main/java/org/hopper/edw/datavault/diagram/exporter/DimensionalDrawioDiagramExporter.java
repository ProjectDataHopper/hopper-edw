...
@DiagramExporter(
    id = "dimensional-drawio",
    name = "Dimensional model Draw.io Exporter",
    description = "Exports a dimensional model as a Draw.io / diagrams.net diagram",
    format = "DRAWIO",
    fileExtension = "drawio",
    fileFilterNames = {"Draw.io Files (*.drawio)"},
    supportedSubjectTypes = {DimensionalModel.class})
public class DimensionalDrawioDiagramExporter extends EdwTextDiagramExporter<DimensionalModel> {
  ...
  public DiagramExportResult export(
    ...
    try {
      graph = ArchitectureGraphFromModel.fromDimensional(model, variables);
    } catch (HopException ignored) {
      // Fall back to a grid layout in the schema renderer.
    }
    String content =
        EdwDrawioExportSupport.export(
            DimensionalSchemaBuilder.build(model), graph, SchemaRenderOptions.from(options));
