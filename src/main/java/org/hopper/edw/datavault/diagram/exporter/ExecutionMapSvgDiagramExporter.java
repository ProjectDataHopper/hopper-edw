...
@DiagramExporter(
    id = "execution-map-svg",
    name = "Execution map SVG Exporter",
    description = "Exports an execution map diagram to Scalable Vector Graphics (SVG)",
    format = "SVG",
    fileExtension = "svg",
    fileFilterNames = {"SVG Files (*.svg)"},
    supportedSubjectTypes = {ExecutionMapDocument.class})
public class ExecutionMapSvgDiagramExporter extends EdwSvgDiagramExporter<ExecutionMapDocument> {
  ...
  public DiagramExportResult export(
    ...
    SvgRenderOptions render = renderOptions(options);
    ...
    String svgXml = ExecutionMapSvgPainter.generateExecutionMapSvg(document, render, variables);
