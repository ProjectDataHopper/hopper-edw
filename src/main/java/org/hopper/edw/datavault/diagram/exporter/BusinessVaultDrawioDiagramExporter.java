...
@DiagramExporter(
    id = "business-vault-drawio",
    name = "Business Vault Draw.io Exporter",
    description = "Exports a Business Vault model as a Draw.io / diagrams.net diagram",
    format = "DRAWIO",
    fileExtension = "drawio",
    fileFilterNames = {"Draw.io Files (*.drawio)"},
    supportedSubjectTypes = {BusinessVaultModel.class})
public class BusinessVaultDrawioDiagramExporter extends EdwTextDiagramExporter<BusinessVaultModel> {
  ...
  public DiagramExportResult export(
    ...
    try {
      graph = ArchitectureGraphFromModel.fromBusinessVault(model, null, variables);
    } catch (HopException ignored) {
      // Fall back to a grid layout in the schema renderer.
    }
    String content =
        EdwDrawioExportSupport.export(
            BusinessVaultSchemaBuilder.build(model), graph, SchemaRenderOptions.from(options));
