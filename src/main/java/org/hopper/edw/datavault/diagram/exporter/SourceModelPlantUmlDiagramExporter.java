...
@DiagramExporter(
    id = "source-model-plantuml",
    name = "Source model PlantUML Exporter",
    description = "Exports a source model as a PlantUML ER diagram",
    format = "PLANTUML",
    fileExtension = "puml",
    fileFilterNames = {"PlantUML Files (*.puml)"},
    supportedSubjectTypes = {SourceModel.class})
public class SourceModelPlantUmlDiagramExporter extends EdwTextDiagramExporter<SourceModel> {
  ...
  public DiagramExportResult export(
    ...
    String content =
        PlantUmlSchemaRenderer.render(
            SourceModelSchemaBuilder.build(model), SchemaRenderOptions.from(options));
