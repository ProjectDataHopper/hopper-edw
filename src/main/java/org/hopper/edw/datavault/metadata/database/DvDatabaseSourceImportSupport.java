...
public final class DvDatabaseSourceImportSupport {
  ...
  public static List<SourceField> importFieldsFromTable(
    ...
    for (IValueMeta vm : rowMeta.getValueMetaList()) {
      ...
      org.hopper.edw.datavault.metadata.DvSqlStringTypeSupport.normalizeStringLength(vm);
      int length = vm.getLength();
      if (org.hopper.edw.datavault.metadata.DvSqlStringTypeSupport.isLargeTextSqlType(nativeType)) {
        length =
            org.hopper.edw.datavault.metadata.DvSqlStringTypeSupport.capacityForSqlStringType(
                nativeType, length);
      }
