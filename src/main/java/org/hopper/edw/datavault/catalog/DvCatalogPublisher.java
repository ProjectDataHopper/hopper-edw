...
public final class DvCatalogPublisher {
  ...
  public static PublishResult publish(
        ...
        log.logBasic(
            "Published DV model registry entry: "
                + CatalogModelRegistrySupport.modelRegistryKey(
                    ...
                    org.hopper.edw.catalog.model.RecordDefinitionType.DV_MODEL));
      }
    } catch (Exception e) {
      errorCount++;
      if (log != null) {
    ...
    try {
      org.hopper.edw.datavault.lineage.LineageSnapshot lineageSnapshot =
          org.hopper.edw.datavault.lineage.DvModelLineageCollector.collect(
              model, variables, metadataProvider, catalogConnectionName);
      org.hopper.edw.datavault.lineage.LineageCatalogPublisher.PublishResult lineageResult =
          org.hopper.edw.datavault.lineage.LineageCatalogPublisher.publish(
      ...
      if (log != null) {
        log.logBasic(
            "Published "
                ...
                + " lineage catalog record(s) for DV model");
      }
    } catch (Exception e) {
      errorCount++;
      if (log != null) {
        log.logError("Failed to publish DV lineage catalog records", e);
      }
    }
  ...
  private static org.hopper.edw.datavault.lineage.LineageCatalogPublisher.CatalogPublishLog
      adaptLog(CatalogPublishLog log) {
    if (log == null) {
      return null;
    }
    return new org.hopper.edw.datavault.lineage.LineageCatalogPublisher.CatalogPublishLog() {
      @Override
  ...
  private static RecordDefinitionType mapTableType(DvTableType tableType) {
    ...
    return switch (tableType) {
      ...
      case REFERENCE -> RecordDefinitionType.DV_REFERENCE;
      case LINKED_TABLE, TABLE_REFERENCE -> RecordDefinitionType.UNKNOWN;
    };
  }
