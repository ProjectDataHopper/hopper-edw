...
- On Hop Web the Gantt tab fills the results pane: the RAP Browser is laid out to the pane (no nested iframe scrollbars from CSS `transform:scale`), the page height tracks the viewport under Fit width, and Gantt rows grow into extra height.
- Live Performance (and Gantt) refresh on Hop Web replaces SVG in the existing RAP Browser instead of `setText` every tick. RAP never fires `ProgressListener.completed` for `setText`, so each update was reloading the iframe, packing the wrap toolbar, and reflowing the whole GUI.
...
### EDW metadata category

- Plugin metadata types appear under **EDW** in the Metadata perspective (tree, overview, and New-type menu) instead of **Other**. Source model service stays under **Servers and Web Services**.
...
- Opening generated documentation on Hop Web uses the same RAP handler as plugin EDW docs (`servicehandler=hopperEdwDocs&file=…`) in the **regular browser**, not the explorer HTML tab. The File explorer context menu has **Open in browser** for HTML (and a documentation folder's `index.html`).
